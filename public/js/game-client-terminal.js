...
function displayMessage(messageData, withAnimation = true) {
    ...
    if (messageData.file_url) {
        if (messageData.file_type && messageData.file_type.startsWith('image/')) {
            ...
            // فایل عادی
            line.innerHTML = `[${timestamp}] ${senderName}: ${messageData.content || ''} <a href="${messageData.file_url}" target="_blank" style="color: var(--accent);">📎 فایل ضمیمه</a>`;
        }
    } else {
        // پیام متنی عادی
        line.innerHTML = `[${timestamp}] ${senderName}: ${messageData.content || ''}`;
    }
...
function createImageWidget(messageData, timestamp, senderName) {
    ...
    return `
        <div>[${timestamp}] ${senderName}: تحلیل تصویری</div>
        <div class="widget image-widget" id="${widgetId}">
...
function createAudioWidget(messageData, timestamp, senderName) {
    ...
    return `
        <div>[${timestamp}] ${senderName}: تحلیل صوتی</div>
        <div class="widget audio-widget" id="${widgetId}">
...
function createVideoWidget(messageData, timestamp, senderName) {
    ...
    return `
        <div>[${timestamp}] ${senderName}: تحلیل ویدیویی</div>
        <div class="widget image-widget" id="${widgetId}">
